...
class WebUser
{
    ...
    public WebUser(string Password, string UserID)
    {
        this.UserID = UserID;
...
public class RedisCache : IDisposable
{
    ...
    public static IEnumerable<RedisKey> Keys
    {
        get
        {
            return GetKeys("*");
    ...
    {
        return ConnectionMultiplexer.Connect("loginservices.redis.cache.windows.net,abortConnect=false,ssl=true,password=RXk52rRjoldJHPtUUlV9UZHJMiS2XhMo8cfpT4GnUvg=");
    });


    private static IEnumerable<RedisKey> GetKeys(string pattern)
    {
        if (pattern == null)
    ...
    private static IServer GetDataCacheServer()
    {
        var endpoints = Connection.GetEndPoints();
        var first = endpoints.First();
        var server = Connection.GetServer(first);
...
public class Redis : IDisposable
{
    ...
    //    try
    //    {
    //        ConnectionMultiplexer connection = ConnectionMultiplexer.Connect("loginservices.redis.cache.windows.net,abortConnect=false,ssl=true");
    //    }
    //    catch (Exception e)
    //    {
