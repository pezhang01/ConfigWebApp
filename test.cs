...
class Test {
	...
	static void Main (string[] args)
	{
        ...
        RedisCache.FlushAll();
        assert((i = RedisCache.KeysLength) == 0, "there should be no keys but there were {0}", i);
        RedisCache.Set("foo", "bar");
        assert((i = RedisCache.KeysLength) == 1, "there should be one key but there were {0}", i);
        RedisCache.Set("foo bär", "bär foo");
        assert((i = RedisCache.KeysLength) == 2, "there should be two keys but there were {0}", i);

        RedisCache.Get("foo");

        //assert(RedisCache.TypeOf("foo") == Redis.KeyType.String, "type is not string");
        //RedisCache.Set("bar", "foo");

        // end connect to Redis Cache in Azure
